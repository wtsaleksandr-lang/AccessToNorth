...
import {
  AlertTriangle, ArrowLeft, ArrowRight, Check, CheckCircle2, FileText, Mail,
  Package, Plane, Plus, Route, Ship, ShieldCheck, Thermometer, TrainFront,
  Trash2, Truck, UploadCloud,
} from "lucide-react";
...
export default function FreightQuote() {
  usePageMeta({
    ...
    description: "Send a carrier-ready freight quote request for ocean, air, truck, rail, or courier shipping. Include cargo dimensions, handling requirements, and documents.",
  ...
  const submit = async () => {
    ...
    try {
      ...
      body.append("request", JSON.stringify({
        ...
        cargoLines: cargoLines.map((line) => ({
          ...line,
      ...
      <main className="flex-1 pt-28 pb-16">
        <div className="container mx-auto max-w-5xl px-4 md:px-6">
          ...
          <div className="mx-auto mb-8 max-w-3xl text-center">
            <Badge className="mb-3 border-0 bg-primary/10 text-primary">Structured RFQ</Badge>
            <h1 className="text-3xl font-extrabold text-slate-950 md:text-4xl" data-testid="text-freight-title">Request a freight quote</h1>
            <p className="mt-4 text-base leading-relaxed text-slate-600 md:text-lg">Create one carrier-ready shipment brief for ocean, air, truck, rail, or courier service. Your request goes to AccessToNorth operations for review.</p>
          </div>
            ...
            <CardContent className="p-5 sm:p-7">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
                      {MODE_OPTIONS.map(({ id, label, detail, icon: Icon }) => (
                        <button key={id} type="button" onClick={() => setField("mode", id)} className={`rounded-xl border p-3 text-left transition ${form.mode === id ? "border-primary bg-primary/5 ring-1 ring-primary/20" : "border-slate-200 bg-white hover:border-slate-300"}`} data-testid={`freight-mode-${id}`}>
              ...
              {step === 1 && (
                <div className="space-y-6" data-testid="freight-step-cargo">
                  ...
                  <div className="grid gap-2 sm:grid-cols-3">
                    ...
                    <label className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 ${form.hazardous ? "border-amber-300 bg-amber-50" : "border-slate-200 bg-white"}`}><input type="checkbox" checked={form.hazardous} onChange={(event) => setField("hazardous", event.target.checked)} /><AlertTriangle className="h-4 w-4 text-amber-600" /><span className="text-sm font-semibold">Hazardous goods</span></label>
                    <label className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 ${form.temperatureControlled ? "border-sky-300 bg-sky-50" : "border-slate-200 bg-white"}`}><input type="checkbox" checked={form.temperatureControlled} onChange={(event) => setField("temperatureControlled", event.target.checked)} /><Thermometer className="h-4 w-4 text-sky-600" /><span className="text-sm font-semibold">Temperature control</span></label>
                  </div>
              ...
              {step === 2 && (
                <div className="space-y-6" data-testid="freight-step-contact">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="mb-3 flex items-center gap-2"><FileText className="h-4 w-4 text-primary" /><p className="text-sm font-bold text-slate-900">Request summary</p></div><div className="grid gap-2 text-sm sm:grid-cols-2"><p><span className="text-slate-500">Route:</span> <strong>{form.origin} → {form.destination}</strong></p><p><span className="text-slate-500">Mode:</span> <strong className="capitalize">{form.mode}</strong></p><p><span className="text-slate-500">Cargo:</span> <strong>{cargoSummary.packages} packages · {cargoSummary.weightKg.toFixed(0)} kg</strong></p><p><span className="text-slate-500">Documents:</span> <strong>{documents.length}</strong></p></div></div>
