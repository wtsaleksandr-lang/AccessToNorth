...
import { CheckCircle2, Copy, KeyRound, LayoutDashboard } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useToast } from "@/hooks/use-toast";
import { TOOL_ACCOUNT_TOKEN_STORAGE_KEY } from "@/lib/toolAccount";

type Summary = { planName: string; status: string; trialEnd: string | null; email: string | null; apiKey?: { key: string | null; prefix: string; alreadyProvisioned: boolean } | null; accountToken?: { token: string; prefix: string } | null };
...
export default function SubscriptionSuccess() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/tool-subscriptions/session/${encodeURIComponent(sessionId)}`).then(async (response) => {
      ...
      if (data.accountToken?.token) window.localStorage.setItem(TOOL_ACCOUNT_TOKEN_STORAGE_KEY, data.accountToken.token);
      setSummary(data);
  ...
  return <div className="min-h-screen bg-slate-50"><main className="mx-auto max-w-2xl px-4 pb-24 pt-32"><section className="rounded-3xl border border-slate-200 bg-white p-7 text-center shadow-sm sm:p-10">
    ...
    <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><Button asChild><a href="/tools/container-calculator">Open calculator</a></Button><Button asChild variant="outline"><a href="/developers/container-loading-dashboard"><LayoutDashboard className="mr-2 h-4 w-4" />Account dashboard</a></Button><Button variant="outline" onClick={manage}>Manage billing</Button></div></>}
  </section></main></div>;
}
