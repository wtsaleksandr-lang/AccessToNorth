...
## Search Console setup — account action required

- [ ] Add a Domain property for `accesstonorth.com` in Google Search Console
- [ ] Verify the Domain property with the DNS TXT record Google provides
- [ ] Submit `https://accesstonorth.com/sitemap.xml`
- [ ] Inspect and request indexing for these initial URLs:
...
- [ ] Review Page indexing, Core Web Vitals, HTTPS, and Manual actions after Google recrawls
...
- [ ] Replace meta-only prerendering with full static HTML rendering for articles, service pages, and tool introductions. The current SPA is indexable, but crawl-visible body HTML would be more reliable and faster for search engines.
- [ ] Add a real expert/reviewer profile with name, role, experience, and a profile page. Do not invent credentials.
- [ ] Add one detailed, original worked example to each major tool page, including screenshots and the assumptions used.
- [ ] Publish supporting articles around each tool's real search terms, then link them back to the tool. Examples: pallet capacity by container type, 20 ft vs 40 ft container selection, Canadian HS-code examples, and truck trailer dimension guides.
...
- [ ] Add an `updatedDate` field to blog posts so sitemap and Article schema show the real revision date
- [ ] Add automated checks for broken internal links and missing image alt text
- [ ] Return a true HTTP 404 for unknown public routes while preserving the branded page
